...
class ModelUpdateService:
    ...
    def _select_sender_ranks_by_pp(self) -> Dict[int, int]:
        """
        Choose one sender rank per PP rank.

        Following ROLL_multi_pipeline, prefer ranks that own sender-side cache:
        dp_rank==0, tp_rank==0, cp_rank==0.
        """
        candidates_by_pp: Dict[int, List[int]] = {}
        for rank, info in enumerate(self.src_cluster.worker_rank_info):
            if info.dp_rank != 0 or info.tp_rank != 0 or info.cp_rank != 0:
                continue
            candidates_by_pp.setdefault(int(info.pp_rank), []).append(int(rank))

        if not candidates_by_pp:
            raise RuntimeError(
                "No sender candidates found for selective sync (expected dp_rank==0 and tp_rank==0 and cp_rank==0)"
            )

        pp_to_sender: Dict[int, int] = {}
        for pp_rank, candidates in candidates_by_pp.items():
            pp_to_sender[int(pp_rank)] = int(sorted(candidates)[0])
        return pp_to_sender
        ...
        for tgt_rank in tgt_dp_ranks:
            for device in self.tgt_cluster.rank2devices[int(tgt_rank)]:
                ...
                if tgt_gpu_key in src_gpu_keys:
                    # NCCL cannot form a group with duplicate physical GPUs. Keep same-GPU targets on IPC path.
                    continue
                tgt_devices.append({"rank": int(tgt_rank), "device": device})
                tgt_ranks_in_group.add(int(tgt_rank))

        safe_sync_id = str(sync_id).replace("/", "_")
        group_name = f"selective_model_update_{safe_sync_id}_pp{src_pp_rank}_src{src_rank}"
    ...
    def sync_selected_workers(self, tgt_dp_ranks: List[int], adapters_to_sync: list[str] | None = None) -> None:
        ...
        pp_to_sender = self._select_sender_ranks_by_pp()
        setup_refs = []
        sync_calls: List[Tuple[int, Optional[dict]]] = []

        # Build and setup groups for leaders first.
        for pp_rank, src_rank in sorted(pp_to_sender.items()):
            ...
            logger.info(
                ...
                f"pipeline_id={self.pipeline_id} sync_id={sync_id} pp_rank={int(pp_rank)} "
                f"src_rank={int(src_rank)} broadcast_tgt_ranks={tgt_ranks_in_group} "
            ...
            if tgt_ranks_in_group:
                ...
                sync_calls.append((int(src_rank), comm_plan))
            else:
                # No broadcast targets (all targets colocated). Selective sync will take the IPC path.
                sync_calls.append((int(src_rank), None))

        # Schedule all train ranks to participate in the final dist.barrier().
        comm_plan_by_rank: Dict[int, Optional[dict]] = {}
        for src_rank, comm_plan in sync_calls:
            comm_plan_by_rank[int(src_rank)] = comm_plan
        ...
        try:
            ...
            for rank, worker in enumerate(self.src_cluster.workers):
                rank_info = self.src_cluster.worker_rank_info[int(rank)]
                is_leader = int(rank) == int(pp_to_sender.get(int(rank_info.pp_rank), -999))
                comm_plan = comm_plan_by_rank.get(int(rank)) if is_leader else None
                sync_refs.append(
        ...
        # Groups are destroyed by selective_sync_active_cache (sender side) before dist.barrier().
        # ncclCommDestroy blocks if called after dist.barrier(), so teardown must happen there.
